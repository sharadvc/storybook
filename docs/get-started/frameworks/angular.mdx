...
Storybook mounts each story with Angular's [`bootstrapApplication`](https://angular.dev/api/platform-browser/bootstrapApplication) function instead of bootstrapping a root `NgModule`. Use the [`applicationConfig`](#application-wide-providers) decorator for providers and other options you would pass to `bootstrapApplication` in a standalone application. Use the [`moduleMetadata`](#angular-dependencies) decorator from `@storybook/angular` for NgModule-style metadata (`imports`, `declarations`, `providers`, and `schemas`) that the story's component tree needs.

<CodeSnippets path="angular-bootstrap-application-and-modulemetadata.md" />
