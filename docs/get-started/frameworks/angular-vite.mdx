...
### bootstrapApplication and moduleMetadata

Storybook mounts each story with Angular's [`bootstrapApplication`](https://angular.dev/api/platform-browser/bootstrapApplication) function instead of bootstrapping a root `NgModule`. Use the [`applicationConfig`](#application-wide-providers) decorator for providers and other options you would pass to `bootstrapApplication` in a standalone application. Use the [`moduleMetadata`](#angular-dependencies) decorator from `@storybook/angular-vite` for NgModule-style metadata (`imports`, `declarations`, `providers`, and `schemas`) that the story's component tree needs.

<CodeSnippets path="angular-vite-bootstrap-application-and-modulemetadata.md" />

<Callout variant="info">

Use `applicationConfig` for application-wide providers, including those from `forRoot()` modules via `importProvidersFrom`. Use `moduleMetadata` when a story needs extra modules, declarations, or providers for its template. Standalone components usually list their own `imports`; add `moduleMetadata` when you still depend on NgModules or on components without a selector.
...
Two things the automigration cannot do for you: a `webpackFinal` hook has to be [rewritten as `viteFinal`](../../builders/vite.mdx#migrating-from-webpack), and direct `.md` imports need Vite's [`?raw` suffix](../../builders/vite.mdx#importing-markdown-files-as-strings).
...
See [Importing Markdown files as strings](../../builders/vite.mdx#importing-markdown-files-as-strings).
